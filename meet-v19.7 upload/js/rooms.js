...
const buildRoomsClassDropdown = (meetClassName, classes) => {
  ...
  classes.forEach((el) => {
    ...
    hook.innerHTML = hook.innerHTML + `<a class="dropdown-item ${flagActive}"  href="#" data-rooms-class-active="${flagActive}" data-class-name-key="${el.name}">${el.name}</a>`;
  });
};
...
const btnRoomsAdd = () => {
  ...
  div.innerHTML = `<div class="col-4 px-0">
                      <input type="text" class="form-control room-url text-center" data-type="name">
                  </div>
                  <div class="col-8 px-0">
