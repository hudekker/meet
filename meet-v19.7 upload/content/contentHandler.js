    ...
    try {
      ...
      if (btnMic && btnVid && !g_joinedFlag && btnHangup) {
        ...
        if (g_myBreakout.settings.newMute == true) {
          ...
          document.querySelector('[data-btn-breakout="spk"]').addEventListener("click", handleSpkBtnClick2);
...
const handleSpkBtnClick = async (event) => {
  try {
    ...
    // First send message SPKBTNCLICKED to popup
    // popup will:
    // a) get current state
    // b) toggle the state
...
const getParticipants2 = () => {
  ppt = [...document.querySelectorAll("[data-requested-participant-id]")]
    .map((el) => {
...
const muteAll = async (myPptId) => {
  ...
  for (let i = 1; i < ppts.length; i++) {
    btn = ppts[i].querySelector("button");
    btn.click();
    // await sleep(1500);
    await sleep(100);
